...
class LabelClassificationExample(dspy.Example):
    def __init__(self, system_prompt, user_query, expected_response):
        self.system_prompt = system_prompt  # The "prompt" column from your CSV
        self.user_query = user_query        # The "question" column from your CSV
        self.expected_response = expected_response  # The "response" column from your CSV
    
    # Property to extract expected labels from response
    @property
...
class LabelPredictor(dspy.Module):
    def __init__(self):
        super().__init__()
        # Updated to use Predict with signature
...
def label_format_metric(example, pred):
    # Extract predicted labels from the pred.labels field
    prediction_text = pred.labels
...
def optimize_phi4_prompts(csv_path, test_size=0.2, random_state=42, save_prompt=True):
    ...
    # Create the optimizer (formerly Teleprompter)
    optimizer = dspy.PromptOptimizer(
...
def test_optimized_model(model, test_queries, system_prompt):
    ...
    for query in test_queries:
        ...
        # Call the model with the optimized system prompt
